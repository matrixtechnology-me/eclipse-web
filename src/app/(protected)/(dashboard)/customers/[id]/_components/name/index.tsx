...
import { updateCustomerNameAction } from "../../_actions/update-customer-name";
import {
  Form,
  FormControl,
  FormField,
...
export const Name: FC<NameProps> = ({
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const result = await updateCustomerNameAction({
      value: data.value,
      customerId,
      tenantId,
    });
    ...
    if ("error" in result) {
      toast.error("Falha na atualização", {
        description:
          "Não foi possível atualizar o nome do usuário. Por favor, tente novamente.",
      });
      return;
    }
